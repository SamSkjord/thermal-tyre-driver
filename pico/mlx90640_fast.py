"""
mlx90640_fast.py - Optimized MLX90640 driver for CircuitPython
===============================================================

Faster alternative to adafruit_mlx90640 library with:
- Reduced polling overhead
- Smarter frame acquisition timing
- Bulk I2C reads
- Minimal retry loops

Based on Adafruit CircuitPython MLX90640 library but optimized for speed.
Original: https://github.com/adafruit/Adafruit_CircuitPython_MLX90640

Performance target: ~150-200ms per frame (vs 670ms in Adafruit library)
"""
...
# Constants from MLX90640 datasheet
eeData = [0] * 832
I2C_READ_LEN = 2048
SCALEALPHA = 0.000001
...
class MLX90640Fast:
    """Optimized interface to the MLX90640 temperature sensor."""
    ...
    def __init__(self, i2c_bus, address=0x33):
        ...
        self._current_subpage = 0
        self._frame_time = 0.125  # Default for 8Hz (will be updated)
    ...
    def refresh_rate(self):
        """How fast the MLX90640 will spit out data"""
        controlRegister = [0]
    ...
    def refresh_rate(self, rate):
        ...
        # Update frame time based on refresh rate
        # At each rate, sensor produces 2 subframes per complete frame
        refresh_rates_hz = [0.5, 1, 2, 4, 8, 16, 32, 64]
        if 0 <= rate < len(refresh_rates_hz):
            hz = refresh_rates_hz[rate]
            self._frame_time = 1.0 / hz  # Time for full frame (2 subframes)

    def getFrame(self, framebuf):
        """
        Read complete frame from sensor (both subframes).

        The MLX90640 uses chess-pattern interleaving - need both subframes
        for a complete 768-pixel image.

        OPTIMIZATION: Fixed I2C reads use correct memory map (separate
        pixel and aux data regions) which should be much faster than
        Adafruit's incorrect contiguous 832-word read.
        """
        emissivity = 0.95
        tr = 23.15
        mlx90640Frame = [0] * 834

        # Read both subframes to get complete image
        # Subframe 0 has pixels at even chess positions
        # Subframe 1 has pixels at odd chess positions
        for _ in range(2):
            status = self._GetFrameDataFast(mlx90640Frame)
            if status < 0:
    ...
    def _GetFrameDataFast(self, frameData):
        """
        Optimized frame data acquisition.

        Key changes from Adafruit:
        1. NO sleep in polling loop (pure spin)
        2. Single read attempt (no retry loop)
        3. Same memory layout as Adafruit for compatibility
        """
        statusRegister = [0]
        controlRegister = [0]

        # OPTIMIZATION 1: Poll without sleep (faster)
        dataReady = 0
        timeout = 5000  # Higher limit since no sleep
        for _ in range(timeout):
            self._I2CReadWords(0x8000, statusRegister)
            dataReady = statusRegister[0] & 0x0008  # Bit 3 = data ready
            if dataReady:
                break
            # NO SLEEP - pure polling for speed

        if not dataReady:
            raise RuntimeError("Timeout waiting for frame data")

        # Clear status register
        self._I2CWriteWord(0x8000, 0x0030)

        # OPTIMIZATION 2: Single read, NO RETRY LOOP
        # (Adafruit has retry loop that reads up to 5 times!)
        # Keep same 832-word read for compatibility
        ...
        frameData[833] = statusRegister[0] & 0x0001  # Bit 0 = subpage
    ...
    def _CalculateTo(self, frameData, emissivity, tr, result):
        """Calculate temperature for each pixel - copied from Adafruit library"""
    ...
    # All parameter extraction methods - unchanged from Adafruit library
    ...
    def _I2CReadWords(self, addr, buffer, *, end=None):
        """
        Read words from I2C device.

        OPTIMIZATION: Uses bulk reads with minimal overhead.
        """
        if end is None:
        ...
        with self.i2c_device as i2c:
            while remainingWords:
                ...
                i2c.write_then_readinto(addrbuf, inbuf, in_end=read_words * 2)
